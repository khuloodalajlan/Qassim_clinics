...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['name']) && $do == 'add') {
        ...
        $stmt = $con->prepare("INSERT into   categorys  ( name ) value(?)  ");
        try {
            //code...
    ...
    if (isset($_POST['name']) && isset($_POST['id']) && $do == 'update') {
        ...
        $stmt = $con->prepare("UPDATE  categorys  set name = ?  where id = ?  ");
        try {
            //code...
    ...
    if (isset($_POST['id']) && $do == 'delete') {
        ...
        $stmt = $con->prepare("DELETE from   categorys  where id = ?  ");
        try {
            //code...
    ...
    <div class="row text-center">
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    <?php
                    $rows = getrows('select * from categorys ');
                    foreach ($rows as $row) {
                        $name = $row['name'];
                        $id = $row['id']; ?>
                        <tr>
                            <form id="deleteform" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
                                ...
                                <th>
                                    <ul class="list-inline mb-0">
                                        ...
                                        <li class="list-inline-item"><a href="javascript:{}" onclick="document.getElementById('deleteform').submit();"><i class="ti-trash"></i></a></li>
                                    </ul>

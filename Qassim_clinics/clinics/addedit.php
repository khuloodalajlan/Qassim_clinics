...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['name']) && $do == 'add') //we can check the existanse of all the field but as long as they are not required we won't check
    {
        ...
        try {
            $stmt = $con->prepare("INSERT into   clinics ( name,image,city_id,addresse_link,addresse,contact_info,user_id,approved,working_hours  ) value(?,?,?,?,?,?,?,?,?)  ");
            $stmt->execute(array($name, $date . $filename, $city_id, $addresse_link, $addresse, $contact_info, $_SESSION['id'], $approved,$working_hours));
            $id = $con->lastInsertId();
    ...
    if (isset($_POST['name']) && isset($_POST['id_clinic']) && $do == 'update') {
        ...
        try {
            if (!empty($_FILES['image']['name'])) {
                ...
                $stmt = $con->prepare("UPDATE    clinics  set working_hours=?, name = ? ,image= ? ,city_id = ? ,addresse_link= ? ,addresse= ? ,contact_info= ?,approved = $approved  where id = ? ");
                $stmt->execute(array($working_hours, $name, $date . $filename, $city_id, $addresse_link, $addresse, $contact_info, $id_clinic));
            } else {
                $stmt = $con->prepare("UPDATE    clinics  set working_hours=?, name = ? , city_id = ? ,addresse_link= ? ,addresse= ? ,contact_info= ?,approved = $approved  where id = ? ");
                $stmt->execute(array($working_hours, $name,   $city_id, $addresse_link, $addresse, $contact_info, $id_clinic));
            }
            ...
            <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                        ...
                        <select name="city_id" id="city_id" class="form-control">
                            ...
                            foreach ($rows as $row) { ?>
                                <option value="<?php echo $row['id'] ?>"
                                 <?php if (isset($get) && $get['city_id'] == $row['id']) { ?> selected <?php } ?>><?php echo $row['name'] ?></option>
                            <?php     }
                            ?>
                    ...
                    <div class="form-group col-sm-4">
                        ...
                        <textarea name="contact_info" id="contact_info" cols="30" rows="5" class="form-control"><?php if (isset($get)) { ?><?php echo $get['contact_info'] ?> <?php } ?></textarea>
             
                    </div>
                    <div class="form-group col-sm-4">
                        <label>working hours</label>
                        <input type="text" name="working_hours" class="form-control" required placeholder="working hours" <?php if (isset($get)) { ?>value="<?php echo $get['working_hours'] ?>" <?php } ?>>
                    </div>
                    ...
                    <div class="form-group col-sm-10">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-5">
                                <label>Price</label>
                                <input type="number" id="service_price" class="form-control" placeholder="Services price">
                            </div>
                            <div class="col-sm-2">
                                <label></label>
